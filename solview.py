...
def vote_accounts(node_address):
    ...
    if data is None or len(data) == 0:
      print("Error: getVoteAccounts call failed. Details: \n{}".format(data))
      return
    cluster_stake = [0,0]
...
def cluster(node_address):
    ...
    data = res.json().get('result')
    if data is None or len(data) == 0:
      print("Error: getClusterNodes call failed. Details: \n{}".format(data))
      return
    cluster_versions = {}
    for node in data:
...
def skip_rates(node_address):
    ...
    data = res.json().get('result')
    if data is None or len(data) == 0:
      print("Error: getBlockProduction call failed. Details: \n{}".format(data))
      return
    skip_rates = []
    total_slots = 0
    produced_slots = 0
...
def performance(node_address):
    ...
    data = res.json().get('result')
    if data is None or len(data) == 0:
      print("Error: getRecentPerformanceSamples call failed. Details: \n{}".format(data))
      return
...
def watch_accounts(node_address, accounts):
    for address in accounts:
      ...
      data = res.json().get('result')
      if data is None or len(data) == 0:
        print("Error: getBalance call failed. Details: \n{}".format(data))
        return
...
def watch_spl_accounts(node_address, spl_accounts):
   for address in spl_accounts:
      ...
      data = res.json().get('result')
      if data is None or len(data) == 0:
        print("Error: getTokenAccountBalance call failed. Details: \n{}".format(data))
        return
